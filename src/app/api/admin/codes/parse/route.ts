...
const schema = z.object({ text: z.string().max(10_000_000), campaignId: z.string().uuid().optional() });
...
export async function POST(request: NextRequest) {
  ...
  try {
    return apiSuccess(await previewCodeImport(parsed.data.text, parsed.data.campaignId));
  } catch (error) {
